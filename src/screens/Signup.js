...
export default function Signup() {
  ...
  const handleClick = async (e) => {
    ...
    const response = await fetch("https://getyourfoodbackend-r22s.onrender.com/api/auth/getlocation", {
      method: 'POST',
      headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch("https://getyourfoodbackend-r22s.onrender.com/api/auth/createuser", {
      // credentials: 'include',
      ...
      <Navbar />
        ...
        <div className='container' >
          <form className='w-50 m-auto mt-5 border bg-dark border-warning rounded' onSubmit={handleSubmit}>
            ...
            <div className="m-3">
              <button type="button" onClick={handleClick} name="geolocation" className=" btn btn-warning text-white">Click for current Location </button>
            </div>
            <div className="m-3">
            ...
            <button type="submit" className="m-3 btn btn-warning text-white">Submit</button>
            <Link to="/login" className="m-3 mx-1 btn btn-success text-white">Already a user</Link>
          </form>
        </div>
      </div>
  )
